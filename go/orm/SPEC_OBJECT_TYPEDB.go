...
func (backRepoSPEC_OBJECT_TYPE *BackRepoSPEC_OBJECT_TYPEStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, spec_object_type *models.SPEC_OBJECT_TYPE) (Error error) {
	...
	if spec_object_typeDB, ok := backRepoSPEC_OBJECT_TYPE.Map_SPEC_OBJECT_TYPEDBID_SPEC_OBJECT_TYPEDB[idx]; ok {
		...
		// commit pointer value spec_object_type.ALTERNATIVE_ID translates to updating the spec_object_type.ALTERNATIVE_IDID
		spec_object_typeDB.ALTERNATIVE_IDID.Valid = true // allow for a 0 value (nil association)
		if spec_object_type.ALTERNATIVE_ID != nil {
			if ALTERNATIVE_IDId, ok := backRepo.BackRepoA_ALTERNATIVE_ID.Map_A_ALTERNATIVE_IDPtr_A_ALTERNATIVE_IDDBID[spec_object_type.ALTERNATIVE_ID]; ok {
				spec_object_typeDB.ALTERNATIVE_IDID.Int64 = int64(ALTERNATIVE_IDId)
				spec_object_typeDB.ALTERNATIVE_IDID.Valid = true
			}
		} else {
			spec_object_typeDB.ALTERNATIVE_IDID.Int64 = 0
			spec_object_typeDB.ALTERNATIVE_IDID.Valid = true
		}

		// commit pointer value spec_object_type.SPEC_ATTRIBUTES translates to updating the spec_object_type.SPEC_ATTRIBUTESID
		spec_object_typeDB.SPEC_ATTRIBUTESID.Valid = true // allow for a 0 value (nil association)
		if spec_object_type.SPEC_ATTRIBUTES != nil {
			if SPEC_ATTRIBUTESId, ok := backRepo.BackRepoA_SPEC_ATTRIBUTES.Map_A_SPEC_ATTRIBUTESPtr_A_SPEC_ATTRIBUTESDBID[spec_object_type.SPEC_ATTRIBUTES]; ok {
				spec_object_typeDB.SPEC_ATTRIBUTESID.Int64 = int64(SPEC_ATTRIBUTESId)
				spec_object_typeDB.SPEC_ATTRIBUTESID.Valid = true
			}
		} else {
			spec_object_typeDB.SPEC_ATTRIBUTESID.Int64 = 0
			spec_object_typeDB.SPEC_ATTRIBUTESID.Valid = true
		}
...
func (spec_object_typeDB *SPEC_OBJECT_TYPEDB) DecodePointers(backRepo *BackRepoStruct, spec_object_type *models.SPEC_OBJECT_TYPE) {
	...
	{
		id := spec_object_typeDB.ALTERNATIVE_IDID.Int64
		if id != 0 {
			tmp, ok := backRepo.BackRepoA_ALTERNATIVE_ID.Map_A_ALTERNATIVE_IDDBID_A_ALTERNATIVE_IDPtr[uint(id)]

			// if the pointer id is unknown, it is not a problem, maybe the target was removed from the front
			if !ok {
				log.Println("DecodePointers: spec_object_type.ALTERNATIVE_ID, unknown pointer id", id)
				spec_object_type.ALTERNATIVE_ID = nil
			} else {
				// updates only if field has changed
				if spec_object_type.ALTERNATIVE_ID == nil || spec_object_type.ALTERNATIVE_ID != tmp {
					spec_object_type.ALTERNATIVE_ID = tmp
				}
			}
		} else {
			spec_object_type.ALTERNATIVE_ID = nil
	...
	{
		id := spec_object_typeDB.SPEC_ATTRIBUTESID.Int64
		if id != 0 {
			tmp, ok := backRepo.BackRepoA_SPEC_ATTRIBUTES.Map_A_SPEC_ATTRIBUTESDBID_A_SPEC_ATTRIBUTESPtr[uint(id)]

			// if the pointer id is unknown, it is not a problem, maybe the target was removed from the front
			if !ok {
				log.Println("DecodePointers: spec_object_type.SPEC_ATTRIBUTES, unknown pointer id", id)
				spec_object_type.SPEC_ATTRIBUTES = nil
			} else {
				// updates only if field has changed
				if spec_object_type.SPEC_ATTRIBUTES == nil || spec_object_type.SPEC_ATTRIBUTES != tmp {
					spec_object_type.SPEC_ATTRIBUTES = tmp
				}
			}
		} else {
			spec_object_type.SPEC_ATTRIBUTES = nil
...
func (spec_object_typeDB *SPEC_OBJECT_TYPEDB) CopyBasicFieldsFromSPEC_OBJECT_TYPE(spec_object_type *models.SPEC_OBJECT_TYPE) {
	...
	spec_object_typeDB.LAST_CHANGE_Data.String = spec_object_type.LAST_CHANGE
...
func (spec_object_typeDB *SPEC_OBJECT_TYPEDB) CopyBasicFieldsToSPEC_OBJECT_TYPE(spec_object_type *models.SPEC_OBJECT_TYPE) {
	...
	spec_object_type.LAST_CHANGE = spec_object_typeDB.LAST_CHANGE_Data.String
...
func (backRepoSPEC_OBJECT_TYPE *BackRepoSPEC_OBJECT_TYPEStruct) RestorePhaseTwo() {
	...
	for _, spec_object_typeDB := range backRepoSPEC_OBJECT_TYPE.Map_SPEC_OBJECT_TYPEDBID_SPEC_OBJECT_TYPEDB {
		...
		if spec_object_typeDB.ALTERNATIVE_IDID.Int64 != 0 {
			spec_object_typeDB.ALTERNATIVE_IDID.Int64 = int64(BackRepoA_ALTERNATIVE_IDid_atBckpTime_newID[uint(spec_object_typeDB.ALTERNATIVE_IDID.Int64)])
			spec_object_typeDB.ALTERNATIVE_IDID.Valid = true
		}

		// reindexing SPEC_ATTRIBUTES field
		if spec_object_typeDB.SPEC_ATTRIBUTESID.Int64 != 0 {
			spec_object_typeDB.SPEC_ATTRIBUTESID.Int64 = int64(BackRepoA_SPEC_ATTRIBUTESid_atBckpTime_newID[uint(spec_object_typeDB.SPEC_ATTRIBUTESID.Int64)])
			spec_object_typeDB.SPEC_ATTRIBUTESID.Valid = true
		}
