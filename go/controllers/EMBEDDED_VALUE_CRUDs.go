...
func (controller *Controller) UpdateEMBEDDED_VALUE(c *gin.Context) {
	...
	if embedded_valueOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), embedded_valueOld, embedded_valueNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), embedded_valueOld, embedded_valueNew, mouseEvent)
