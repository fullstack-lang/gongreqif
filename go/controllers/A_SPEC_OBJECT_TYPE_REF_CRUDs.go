...
func (controller *Controller) UpdateA_SPEC_OBJECT_TYPE_REF(c *gin.Context) {
	...
	if a_spec_object_type_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_spec_object_type_refOld, a_spec_object_type_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_spec_object_type_refOld, a_spec_object_type_refNew, mouseEvent)
