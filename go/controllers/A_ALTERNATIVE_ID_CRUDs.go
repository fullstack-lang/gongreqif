...
func (controller *Controller) UpdateA_ALTERNATIVE_ID(c *gin.Context) {
	...
	if a_alternative_idOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_alternative_idOld, a_alternative_idNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_alternative_idOld, a_alternative_idNew, mouseEvent)
