...
func (controller *Controller) GetA_RELATION_GROUP_TYPE_REFs(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetA_RELATION_GROUP_TYPE_REFs", "Name", stackPath)
		}
	}
	...
	_, err := db.Find(&a_relation_group_type_refDBs)
	if err != nil {
...
func (controller *Controller) PostA_RELATION_GROUP_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostA_RELATION_GROUP_TYPE_REFs", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	_, err = db.Create(&a_relation_group_type_refDB)
	if err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
...
func (controller *Controller) GetA_RELATION_GROUP_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetA_RELATION_GROUP_TYPE_REF", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&a_relation_group_type_refDB, c.Param("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateA_RELATION_GROUP_TYPE_REF(c *gin.Context) {
	...
	_, err := db.First(&a_relation_group_type_refDB, c.Param("id"))
	...
	db, _ = db.Model(&a_relation_group_type_refDB)
	...
	if a_relation_group_type_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_relation_group_type_refOld, a_relation_group_type_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_relation_group_type_refOld, a_relation_group_type_refNew, mouseEvent)
...
func (controller *Controller) DeleteA_RELATION_GROUP_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteA_RELATION_GROUP_TYPE_REF", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	db.Unscoped()
	db.Delete(&a_relation_group_type_refDB)
