...
func (controller *Controller) UpdateRELATION_GROUP(c *gin.Context) {
	...
	if relation_groupOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), relation_groupOld, relation_groupNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), relation_groupOld, relation_groupNew, mouseEvent)
