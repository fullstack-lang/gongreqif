...
func (controller *Controller) UpdateA_SPECIFICATION_TYPE_REF(c *gin.Context) {
	...
	if a_specification_type_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_specification_type_refOld, a_specification_type_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_specification_type_refOld, a_specification_type_refNew, mouseEvent)
