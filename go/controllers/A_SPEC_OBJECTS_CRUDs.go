...
func (controller *Controller) UpdateA_SPEC_OBJECTS(c *gin.Context) {
	...
	if a_spec_objectsOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_spec_objectsOld, a_spec_objectsNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_spec_objectsOld, a_spec_objectsNew, mouseEvent)
