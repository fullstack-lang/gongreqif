...
func (controller *Controller) UpdateStaticWebSiteParagraph(c *gin.Context) {
	...
	if staticwebsiteparagraphOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsiteparagraphOld, staticwebsiteparagraphNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsiteparagraphOld, staticwebsiteparagraphNew, mouseEvent)
