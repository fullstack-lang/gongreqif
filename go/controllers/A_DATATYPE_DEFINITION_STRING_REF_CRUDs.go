...
func (controller *Controller) UpdateA_DATATYPE_DEFINITION_STRING_REF(c *gin.Context) {
	...
	if a_datatype_definition_string_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_datatype_definition_string_refOld, a_datatype_definition_string_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_datatype_definition_string_refOld, a_datatype_definition_string_refNew, mouseEvent)
