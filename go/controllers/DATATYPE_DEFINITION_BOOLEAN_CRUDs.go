...
func (controller *Controller) UpdateDATATYPE_DEFINITION_BOOLEAN(c *gin.Context) {
	...
	if datatype_definition_booleanOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), datatype_definition_booleanOld, datatype_definition_booleanNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), datatype_definition_booleanOld, datatype_definition_booleanNew, mouseEvent)
