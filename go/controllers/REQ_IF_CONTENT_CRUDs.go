...
func (controller *Controller) UpdateREQ_IF_CONTENT(c *gin.Context) {
	...
	if req_if_contentOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), req_if_contentOld, req_if_contentNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), req_if_contentOld, req_if_contentNew, mouseEvent)
