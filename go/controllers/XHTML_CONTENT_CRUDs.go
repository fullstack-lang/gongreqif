...
func (controller *Controller) UpdateXHTML_CONTENT(c *gin.Context) {
	...
	if xhtml_contentOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), xhtml_contentOld, xhtml_contentNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), xhtml_contentOld, xhtml_contentNew, mouseEvent)
