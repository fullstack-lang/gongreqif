...
func (controller *Controller) UpdateRenderingConfiguration(c *gin.Context) {
	...
	if renderingconfigurationOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), renderingconfigurationOld, renderingconfigurationNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), renderingconfigurationOld, renderingconfigurationNew, mouseEvent)
