...
func (controller *Controller) UpdateA_DATATYPE_DEFINITION_INTEGER_REF(c *gin.Context) {
	...
	if a_datatype_definition_integer_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_datatype_definition_integer_refOld, a_datatype_definition_integer_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_datatype_definition_integer_refOld, a_datatype_definition_integer_refNew, mouseEvent)
