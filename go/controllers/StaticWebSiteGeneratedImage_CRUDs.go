...
func (controller *Controller) UpdateStaticWebSiteGeneratedImage(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("UpdateStaticWebSiteGeneratedImage", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if staticwebsitegeneratedimageOld != nil {
		models.AfterUpdateFromFront(backRepo.GetStage(), staticwebsitegeneratedimageOld, staticwebsitegeneratedimageNew)
	}
