...
func (controller *Controller) UpdateStaticWebSiteGeneratedImage(c *gin.Context) {
	...
	if staticwebsitegeneratedimageOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsitegeneratedimageOld, staticwebsitegeneratedimageNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsitegeneratedimageOld, staticwebsitegeneratedimageNew, mouseEvent)
