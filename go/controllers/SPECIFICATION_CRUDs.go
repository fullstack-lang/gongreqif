...
func (controller *Controller) UpdateSPECIFICATION(c *gin.Context) {
	...
	if specificationOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), specificationOld, specificationNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), specificationOld, specificationNew, mouseEvent)
