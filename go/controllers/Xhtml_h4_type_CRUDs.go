...
func (controller *Controller) GetXhtml_h4_types(c *gin.Context) {
	...
	query := db.Find(&xhtml_h4_typeDBs)
	if query.Error != nil {
...
func (controller *Controller) PostXhtml_h4_type(c *gin.Context) {
	...
	query := db.Create(&xhtml_h4_typeDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) GetXhtml_h4_type(c *gin.Context) {
	...
	if err := db.First(&xhtml_h4_typeDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateXhtml_h4_type(c *gin.Context) {
	...
	query := db.First(&xhtml_h4_typeDB, c.Param("id"))
	...
	query = db.Model(&xhtml_h4_typeDB).Updates(xhtml_h4_typeDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) DeleteXhtml_h4_type(c *gin.Context) {
	...
	db.Unscoped().Delete(&xhtml_h4_typeDB)
