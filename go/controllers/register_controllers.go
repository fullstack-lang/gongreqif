...
func registerControllers(r *gin.Engine) {
	...
	{ // insertion point for registrations
		v1.GET("/v1/tests", GetController().GetTests)
		v1.GET("/v1/tests/:id", GetController().GetTest)
		v1.POST("/v1/tests", GetController().PostTest)
		v1.PATCH("/v1/tests/:id", GetController().UpdateTest)
		v1.PUT("/v1/tests/:id", GetController().UpdateTest)
		v1.DELETE("/v1/tests/:id", GetController().DeleteTest)
...
func (controller *Controller) onWebSocketRequestForCommitFromBackNb(c *gin.Context) {
	...
	// log.Println("Stack github.com/fullstack-lang/gongreqif/go, onWebSocketRequestForCommitFromBackNb")
	...
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gongreqif/go, Unkown stack", stackPath)
	}
...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	// log.Println("Stack github.com/fullstack-lang/gongreqif/go, onWebSocketRequestForBackRepoContent, first sent back repo of", stackPath)
	if err != nil {
		log.Println("client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	...
	for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
		...
		// log.Println("Stack github.com/fullstack-lang/gongreqif/go, onWebSocketRequestForBackRepoContent, sent back repo of", stackPath)
