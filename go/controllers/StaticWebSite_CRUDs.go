...
func (controller *Controller) UpdateStaticWebSite(c *gin.Context) {
	...
	if staticwebsiteOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsiteOld, staticwebsiteNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), staticwebsiteOld, staticwebsiteNew, mouseEvent)
