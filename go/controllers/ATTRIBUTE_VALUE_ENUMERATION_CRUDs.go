...
func (controller *Controller) UpdateATTRIBUTE_VALUE_ENUMERATION(c *gin.Context) {
	...
	if attribute_value_enumerationOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_value_enumerationOld, attribute_value_enumerationNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_value_enumerationOld, attribute_value_enumerationNew, mouseEvent)
