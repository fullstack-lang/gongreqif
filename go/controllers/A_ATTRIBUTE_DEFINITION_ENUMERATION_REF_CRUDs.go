...
func (controller *Controller) UpdateA_ATTRIBUTE_DEFINITION_ENUMERATION_REF(c *gin.Context) {
	...
	if a_attribute_definition_enumeration_refOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_attribute_definition_enumeration_refOld, a_attribute_definition_enumeration_refNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_attribute_definition_enumeration_refOld, a_attribute_definition_enumeration_refNew, mouseEvent)
