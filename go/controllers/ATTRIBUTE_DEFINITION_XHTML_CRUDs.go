...
func (controller *Controller) UpdateATTRIBUTE_DEFINITION_XHTML(c *gin.Context) {
	...
	if attribute_definition_xhtmlOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_definition_xhtmlOld, attribute_definition_xhtmlNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_definition_xhtmlOld, attribute_definition_xhtmlNew, mouseEvent)
