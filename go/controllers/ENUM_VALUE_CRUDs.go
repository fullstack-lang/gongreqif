...
func (controller *Controller) UpdateENUM_VALUE(c *gin.Context) {
	...
	if enum_valueOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), enum_valueOld, enum_valueNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), enum_valueOld, enum_valueNew, mouseEvent)
