...
func (controller *Controller) UpdateA_SPECIFIED_VALUES(c *gin.Context) {
	...
	if a_specified_valuesOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_specified_valuesOld, a_specified_valuesNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), a_specified_valuesOld, a_specified_valuesNew, mouseEvent)
