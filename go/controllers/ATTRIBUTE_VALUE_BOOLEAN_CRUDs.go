...
func (controller *Controller) GetATTRIBUTE_VALUE_BOOLEANs(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetATTRIBUTE_VALUE_BOOLEANs", "Name", stackPath)
		}
	}
	...
	_, err := db.Find(&attribute_value_booleanDBs)
	if err != nil {
...
func (controller *Controller) PostATTRIBUTE_VALUE_BOOLEAN(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostATTRIBUTE_VALUE_BOOLEANs", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	_, err = db.Create(&attribute_value_booleanDB)
	if err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
...
func (controller *Controller) GetATTRIBUTE_VALUE_BOOLEAN(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetATTRIBUTE_VALUE_BOOLEAN", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&attribute_value_booleanDB, c.Param("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateATTRIBUTE_VALUE_BOOLEAN(c *gin.Context) {
	...
	_, err := db.First(&attribute_value_booleanDB, c.Param("id"))
	...
	db, _ = db.Model(&attribute_value_booleanDB)
	...
	if attribute_value_booleanOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_value_booleanOld, attribute_value_booleanNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attribute_value_booleanOld, attribute_value_booleanNew, mouseEvent)
...
func (controller *Controller) DeleteATTRIBUTE_VALUE_BOOLEAN(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteATTRIBUTE_VALUE_BOOLEAN", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	db.Unscoped()
	db.Delete(&attribute_value_booleanDB)
