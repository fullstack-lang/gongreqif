...
func (controller *Controller) GetXhtml_h1_types(c *gin.Context) {
	...
	query := db.Find(&xhtml_h1_typeDBs)
	if query.Error != nil {
...
func (controller *Controller) PostXhtml_h1_type(c *gin.Context) {
	...
	query := db.Create(&xhtml_h1_typeDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) GetXhtml_h1_type(c *gin.Context) {
	...
	if err := db.First(&xhtml_h1_typeDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateXhtml_h1_type(c *gin.Context) {
	...
	query := db.First(&xhtml_h1_typeDB, c.Param("id"))
	...
	query = db.Model(&xhtml_h1_typeDB).Updates(xhtml_h1_typeDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) DeleteXhtml_h1_type(c *gin.Context) {
	...
	db.Unscoped().Delete(&xhtml_h1_typeDB)
