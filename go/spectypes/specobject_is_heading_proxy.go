...
func (proxy *ButtonToggleShowSpecObjectTypeIsHeadingProxy) ButtonUpdated(
	...
	showInHeading, ok := proxy.stager.Map_SPEC_OBJECT_TYPE_isHeading[proxy.specObjectType]

	if !ok {
		log.Fatalln("Unknown specificiation in map", proxy.specObjectType.Name)
	}

	proxy.stager.Map_SPEC_OBJECT_TYPE_isHeading[proxy.specObjectType] = !showInHeading
